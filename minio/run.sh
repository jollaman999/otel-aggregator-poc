...
echo "[*] Checking if buckets 'loki-data' and 'thanos-metrics' exists..."
...
if [[ $BUCKET_HTTP_STATUS =~ ^2[0-9][0-9]$ ]] && echo "$BUCKET_RESPONSE" | grep -q '"name":"thanos-metrics"'; then
    echo "[*] Bucket 'thanos-metrics' already exists. Skipping bucket creation."
else
    echo "[*] Creating bucket 'thanos-metrics'..."
    HTTP_STATUS=$(curl -s -w "%{http_code}" -b $RUN_PATH/minio-cookie -XPOST \
        "http://127.0.0.1:9001/api/v1/buckets" \
        -H 'Accept: */*' \
        -H 'Content-Type: application/json' \
        -d '{"name":"thanos-metrics","versioning":{"enabled":false,"excludePrefixes":[],"excludeFolders":false},"locking":false}' \
        -o /dev/null)

    if [[ $HTTP_STATUS =~ ^2[0-9][0-9]$ ]]; then
        echo "[*] Successfully created bucket 'thanos-metrics'!"
    else
        echo "[!] Failed to create bucket 'thanos-metrics'. HTTP Status: $HTTP_STATUS"
    fi
fi
